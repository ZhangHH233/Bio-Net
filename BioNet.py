...
if __name__ == "__main__":
    ...
    # 验证输出维度是否正确
    assert seg_pred.shape == (batch_size, num_classes, height, width), "Segmentation output shape mismatch"
    assert gms_out.shape == (batch_size, gms_channels, height, width), "GMS output shape mismatch"
    assert bio_out.shape == (batch_size, 1), "Biomarker output shape mismatch"
